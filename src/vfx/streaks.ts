...
const RIBBON_GEOMETRY = new BoxGeometry(0.025, 0.025, 1.6, 1, 1, 32);
...
void main() {
  vT = position.z;
  float wave = sin(position.z * 5.0 + uTime * 3.0 + uPhase) * 0.24;
  float wave2 = cos(position.z * 3.5 - uTime * 2.2 + uPhase) * 0.16;
...
void main() {
  ...
  float edgeFade = 1.0 - smoothstep(0.55, 0.8, abs(vT));
 ...
 */
export function createStreakRibbons(count = 8): StreakRibbons {
